...
function checkAnswer(answer) {
  ...
  // It doesn't matter if the answer is correct or not. We still need to go to the next question
  nextQuestion()
}
...
function nextQuestion() {
  ...
  if (activeQuestion < lastQuestion) {
    activeQuestion++
    // I am calling renderQuestion again to use the new value of
    // active question ie
    renderQuestion()
  } else {
    clearInterval(TIMER)
...
function renderScore() {
  ...
  ScoreContainer.innerHTML += `<h2>Number of Correctly Answered Questions: ${score}`
}
